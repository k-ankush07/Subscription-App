...
function resolveVariantDiscountForCycle(
  ...
  let baseAmount, baseType, afterActive, afterAmount, afterType, afterThreshold;

  if (mode === "CUSTOM") {
    baseAmount = Number(variant.discountAmount || 0);
    baseType = variant.discountType || "PERCENTAGE";
    afterActive = !!variant.changeDiscountAfterOrders;
    afterAmount = Number(variant.discountAmount2 || 0);
    afterType = variant.discountType2 || "PERCENTAGE";
    afterThreshold = Number(variant.afterOrders || 0);
  } else {
    if (!settings?.giveDiscount) return null;
    baseAmount = Number(settings.discountAmount || 0);
    baseType = settings.discountType || "PERCENTAGE";
    afterActive = !!settings.changeDiscountAfterOrders;
    afterAmount = Number(settings.afterDiscountValue || 0);
    afterType = settings.afterDiscountType || "PERCENTAGE";
    afterThreshold = Number(settings.afterOrders || 0);
  }
  ...
  return {
    ...
    sourceVariantId: variant.variantsId,
    adjustmentType,
    adjustmentValue,
    after: useAfter ? afterThreshold : 0,
    __phase: useAfter ? "after" : "before",
  };
}
...
function collectActionsForCycle(
  ...
  const isManualBaseOverride = !!settings.afterDiscountIsManualOverride;
  const lineFixedPrices =
    settings.lineFixedPrices && typeof settings.lineFixedPrices === "object"
      ? settings.lineFixedPrices
      : {};

  let anyVariantDiscountAction = false;
  const variantActionsByVariantId = {};

  if (hasPerVariantProducts) {
    for (const product of settings.products) {
      for (const variant of product.variants || []) {
        if (
          isManualBaseOverride &&
          baseVariantId &&
          variant.variantsId === baseVariantId
        ) {
          continue;
        }
        if (lineFixedPrices[variant.variantsId] != null) {
          continue; // manual per-line override wins, catalog discount skip
        }
        const variantAction = resolveVariantDiscountForCycle(
          product,
          variant,
          settings,
          cycleIndex,
        ...
        if (variantAction) {
          variantActionsByVariantId[variant.variantsId] = variantAction;
          anyVariantDiscountAction = true;
        }
      }
    }
  }

  // Manual per-line fixed-price overrides (Edit page se set kiye gaye)
  for (const [variantId, price] of Object.entries(lineFixedPrices)) {
    variantActionsByVariantId[variantId] = {
      type: "VARIANT_DISCOUNT_CHANGE",
      sourceVariantId: variantId,
      adjustmentType: "FIXED_PRICE",
      adjustmentValue: Number(price),
      after: 0,
      __phase: "manual",
  ...
  for (const action of Object.values(variantActionsByVariantId)) {
    actions.push(action);
  }

  if (!anyVariantDiscountAction || isManualBaseOverride) {
  ...
  try {
    for (const action of orderedActions) {
      ...
      if (action.type === "PRODUCT_SWAP" || action.type === "VARIANT_SWAP") {
        ...
        if (activeManualForSwap.length > 0) {
          const preManualPrice =
            recalculatedPrice != null
              ? Number(recalculatedPrice)
              : basePriceForThisSwap;
          recalculatedPrice = applyManualDiscountsToPrice(
            preManualPrice,
            activeManualForSwap,
          ).toFixed(2);
        }
      ...
      if (action.type === "DISCOUNT_CHANGE") {
        ...
        newPrice = Math.max(0, newPrice).toFixed(2);
        // NEW — manual discounts native/custom base-line discount ke upar apply
        const activeManualForBase = getActiveManualDiscountsForLine(
          extraSettings,
          cycleIndex,
          draftLines[0]?.variantId,
        );
        if (activeManualForBase.length > 0) {
          newPrice = applyManualDiscountsToPrice(
            newPrice,
            activeManualForBase,
          ).toFixed(2);
        }
      ...
      if (action.type === "VARIANT_DISCOUNT_CHANGE") {
        ...
        if (!targetLine) {
          console.warn(
            `[applyActionsToCycle] VARIANT_DISCOUNT_CHANGE skipped — configured variant doesn't match this subscription`,
          );
          action.__skippedReason =
            "Configured variant doesn't match this subscription's lines — discount not applied.";
          continue;
        }
        if (removedLineIds.has(targetLine.id)) {
          console.warn(
            `[applyActionsToCycle] VARIANT_DISCOUNT_CHANGE skipped — target line already removed this cycle`,
          );
          action.__skippedReason =
            "Target line was already removed by another action this cycle.";
          continue;
        ...
        // NEW — manual discounts is variant ke apne discount/fixed-price ke upar apply
        const activeManualForVariant = getActiveManualDiscountsForLine(
          extraSettings,
          cycleIndex,
          targetLine.variantId,
        );
        if (activeManualForVariant.length > 0) {
          newPrice = applyManualDiscountsToPrice(
            newPrice,
            activeManualForVariant,
          ).toFixed(2);
        }

        const res = await admin.graphql(
          `
          mutation updateVariantLinePrice($draftId: ID!, $lineId: ID!, $price: Decimal!) {
            subscriptionDraftLineUpdate(draftId: $draftId, lineId: $lineId, input: { currentPrice: $price }) {
              userErrors { field message }
            }
          }
        ...
        if (data.errors)
          throw new Error(
            `VARIANT_DISCOUNT_CHANGE failed (GraphQL): ${data.errors[0]?.message}`,
          );
        const errors = data.data?.subscriptionDraftLineUpdate?.userErrors;
        if (errors?.length)
          throw new Error(
      ...
      if (action.type === "ADD_PRODUCT") {
        ...
        // NEW — manual discounts is added line ke price ke upar apply
        const activeManualForAdd = getActiveManualDiscountsForLine(
          extraSettings,
          cycleIndex,
          action.variantId,
        );
        if (activeManualForAdd.length > 0) {
          addPrice = applyManualDiscountsToPrice(
            Number(addPrice),
            activeManualForAdd,
          ).toFixed(2);
        }
      ...
      if (lineId && !removedLineIds.has(lineId)) {
        const baseLineVariantId = draftLines[0]?.variantId;
        const hasBaseLinePriceAction = orderedActions.some(
          (a) =>
            a.type === "DISCOUNT_CHANGE" ||
            a.type === "VARIANT_SWAP" ||
            (a.type === "VARIANT_DISCOUNT_CHANGE" &&
              a.sourceVariantId === baseLineVariantId),
        );
        if (!hasBaseLinePriceAction) {
          const activeManualForUntouchedBase = getActiveManualDiscountsForLine(
            extraSettings,
            cycleIndex,
            baseLineVariantId,
          );
          if (activeManualForUntouchedBase.length > 0) {
            const finalPrice = applyManualDiscountsToPrice(
              effectiveBasePrice,
              activeManualForUntouchedBase,
            ).toFixed(2);

            const res = await admin.graphql(
              `
            mutation updateLinePriceManualOnly($draftId: ID!, $lineId: ID!, $price: Decimal!) {
              subscriptionDraftLineUpdate(draftId: $draftId, lineId: $lineId, input: { currentPrice: $price }) {
                userErrors { field message }
              }
            }
            `,
              { variables: { draftId, lineId, price: finalPrice } },
            );
            const data = await res.json();
            if (data.errors)
              throw new Error(
                `Manual discount apply failed (GraphQL): ${data.errors[0]?.message}`,
              );
            const errors = data.data?.subscriptionDraftLineUpdate?.userErrors;
            if (errors?.length)
              throw new Error(
                `Manual discount apply failed: ${errors[0].message}`,
              );
          }
        }
      }
...
function updateAutomationVariantQuantity(
  ...
  if (action.type === "swap" && variantId) {
    if (
      !action.variantQuantities ||
      typeof action.variantQuantities !== "object"
    ) {
      action.variantQuantities = {};
    }
...
function setBaseLineFixedPrice(settings, price) {
...
async function getContractPreview(
  ...
  const variantDiscountActions = rawActionsForNextCycle.filter(
    (a) => a.type === "VARIANT_DISCOUNT_CHANGE",
  );
  ...
  const hasRealDiscount =
    ...
    !discountAction.__manualOverride &&
...
const shopCurrencyCode = await getShopCurrencyCode(admin);
  ...
  for (const line of allLines) {
    ...
    if (isSwapTarget && swapAction.variantId) {
    ...
    } else if (variantDiscountActions.find((a) => actionMatchesLine(a, line))) {
      ...
      if (matchedVariantDiscount.adjustmentType === "PERCENTAGE") {
        discounted =
          effectiveBase -
          (effectiveBase * Number(matchedVariantDiscount.adjustmentValue)) /
            100;
      } else if (
      ...
      if (!discountAction.__default) {
        if (discountAction.adjustmentType === "PERCENTAGE") {
          ...
          discounted = Number(discountAction.adjustmentValue);
        } else {
  ...
  for (const action of addProductActions) {
    ...
    lineItems.push({
      ...
      manualDiscounts: activeManualDiscountsForAdd.map((d) => ({
        id: d.id,
        name: d.name,
        adjustmentType: d.adjustmentType,
        adjustmentValue: d.adjustmentValue,
...
function getActiveManualDiscountsForLine(settings, cycleIndex, variantId) {
  cycleIndex = Number(cycleIndex);
  const all = Array.isArray(settings?.manualDiscounts)
    ? settings.manualDiscounts
    : [];
  return all.filter((d) => {
