...
// CHANGED — added optional `cycleDate` param (7th arg). When provided, the
// draft-open mutation (`subscriptionBillingCycleContractEdit`) selects the
// cycle by date instead of by index. This fixes:
//   "subscriptionBillingCycleContractEdit failed: Billing cycle selector is invalid"
// which Shopify throws when an `index` selector is used for a cycle that
// hasn't been realized yet (no prior billing attempt / edit on that cycle —
// most commonly the very first upcoming cycle right after contract
// creation). All cleanup calls (`clearBillingCycleEdit`) also now forward
// the same date so retries during error-handling use the same, reliable
// selector.
async function applyActionsToCycle(
  admin,
  contractId,
