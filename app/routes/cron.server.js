...
export function startBillingCycleCron() {
  ...
  cron.schedule("* * * * *", async () => {
    ...
    console.log("process.env https://ton-product-assure-penalties.trycloudflare.com")
    try {
      const res = await fetch(`https://ton-product-assure-penalties.trycloudflare.com/api/process-billing-cycles`, {
        method: "POST",
