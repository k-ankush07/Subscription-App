...
//  CycleCard
// CHANGED — accepts `minOrders`: the smallest value this cycle's
// "After # of orders" is allowed to have (previous cycle's orders + 1,
// or 1 for the very first cycle). Enforced two ways:
//  1. `min={minOrders}` on the TextField (blocks the native spinner/typing
//     hint, though a plain <input type="number"> min doesn't block free
//     typing on its own).
//  2. `onChange` clamps any typed value up to minOrders — so the user
//     genuinely cannot get the field below minOrders (never < prev+1, never 0).
function CycleCard({
  cycle,
  minOrders,
  onDelete,
  ...
  const effectiveMin = minOrders ?? 1;
        ...
        <InlineStack align="start" blockAlign="center" gap="200">
          ...
          <div style={{ width: 80 }}>
            <TextField
              ...
              value={String(cycle.orders)}
              onChange={(v) => {
                // CHANGED — clamp: never below effectiveMin (so never below
                // previous cycle's orders + 1, and never 0/blank-parsed-to-0).
                const parsed = parseInt(v, 10);
                const next = Number.isNaN(parsed) ? effectiveMin : parsed;
                onUpdateOrders(Math.max(effectiveMin, next));
              }}
              autoComplete="off"
              min={effectiveMin}
            />
...
useEffect(() => {
  ...
  // CHANGED — new cycle's "after # of orders" now defaults to
  // (last VISIBLE cycle's orders + 1) instead of a hardcoded 1. So the
  // very first cycle still starts at 1, but the 2nd cycle defaults to 2,
  // 3rd to (2nd's current value + 1), etc. — always strictly increasing.
  //
  // Uses `cycles.filter(c => c.actions.length > 0)` — NOT the raw `cycles`
  // array — because a cycle is created empty and only becomes "real" once
  // resolveActionType actually adds an action to it. If the resource
  // picker gets cancelled (or nothing is selected), that cycle stays
  // empty and invisible in the UI, but it used to still sit in `cycles`
  // and get picked up as "lastCycle" next time — silently bumping the
  // next default by an extra +1 (e.g. 1, then a cancelled attempt at 2,
  // then a real add landing on 3 instead of 2).
  const handleMainAddAction = (actionType) => {
    const newCycleId = Date.now();
    const visible = cycles.filter((c) => c.actions.length > 0);
    const lastCycle = visible[visible.length - 1];
    const nextOrders = lastCycle ? (Number(lastCycle.orders) || 1) + 1 : 1;

    setCycles((prev) => [
      ...prev,
      { id: newCycleId, orders: nextOrders, actions: [] },
    ]);
    setShowMainDropdown(false);

    // defer so the new cycle is in state before resolving
    setTimeout(() => {
      resolveActionType(actionType, newCycleId).then(() => {
        // CHANGED — cleanup: agar picker cancel ho gaya ya koi item select
        // nahi hua, ye cycle khaali reh jaata hai. Aise stray khaali
        // cycles ko turant hata do — warna agli baar ka default order
        // isi khaali cycle se galat compute hoga.
        setCycles((prev) =>
          prev.filter((c) => c.id !== newCycleId || c.actions.length > 0),
        );
      });
    }, 0);
  };
  ...
  //  Render
  const visibleCycles = cycles.filter((cycle) => cycle.actions.length > 0);
      ...
      {sellingPlan.Automation && (
        <>
          {visibleCycles.map((cycle, idx) => {
            // CHANGED — each visible cycle's minimum allowed "orders" value
            // is the previous visible cycle's current orders + 1 (or 1 for
            // the first one). Passed down so CycleCard can enforce it live —
            // this stays correct even if the user later edits an earlier
            // cycle's value up or down (as long as it doesn't go below its
            // own min), because we recompute from `cycle.orders`, not a
            // stored/frozen number.
            const prevCycle = idx > 0 ? visibleCycles[idx - 1] : null;
            const minOrders = prevCycle
              ? (Number(prevCycle.orders) || 1) + 1
              : 1;
