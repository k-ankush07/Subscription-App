...
function ActionCard({
  ...
  // FIX (Bug 1): productName should ALWAYS show the product's own name,
  // never the variant name. Previously `sourceVariantName` was checked
  // first, which meant a "variant swap" action showed the variant title
  // as the heading and the actual product name never appeared anywhere.
  const productName =
    action.sourceProductName || // swap (product-level or variant-level)
    action.productName || // add / remove product
    "Unknown product";

  // Variant name(s) shown separately as tag(s) below the product name —
  // this is unaffected by the productName fix above.
  const singleVariantName =
    action.sourceVariantName || action.variantName || null;
  const displayImage = action.imageUrl || null;
  ...
  const subLabel =
    action.sourceVariantName || action.sourceVariantId
      ? null // variant swap — no extra label needed, variant tag already shown
      : action.type === "swap"
        ? "Will match all variants" // only for product-level swap
        : null;

  // FIX (Bug 2, UI-side warning): if more than one destination — or more
  // than one variant across destinations — is configured, only the FIRST
  // one is actually applied by the backend today. Surface that clearly in
  // the UI instead of silently dropping the rest.
  const totalConfiguredDestVariants = (action.dests || []).reduce(
    (sum, d) => sum + (d.variantIds?.length || (d.variantNames?.length ? d.variantNames.length : 1)),
    0,
  );
  const hasMultipleDestTargets = totalConfiguredDestVariants > 1;
        ...
        <>
          ...
          {isSwap && (
            <div style={{ marginTop: 10 }}>
              ...
              {/* FIX (Bug 2, UI warning): only the first configured
                  destination/variant is applied by the backend. Tell the
                  merchant explicitly instead of letting them assume all
                  configured destinations will be used. */}
              {hasMultipleDestTargets && (
                <Text variant="bodySm" tone="caution">
                  <span style={styles.warningNote}>
                    ⚠ Only the first destination/variant below will actually
                    be applied. Extra destinations are ignored — remove them
                    to avoid confusion.
                  </span>
                </Text>
              )}
