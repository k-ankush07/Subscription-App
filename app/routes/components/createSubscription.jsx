...
function getMinOrderDate(interval, intervalCount) {
  const min = new Date();
  if (interval === "DAY") {
    const count = Math.max(Number(intervalCount) || 1, 7);
...
function CreateSubscription({ currencyCode, shop }) {
  ...
  const [intervalCount, setIntervalCount] = useState(1);
  const [interval, setInterval] = useState("MONTH");
  const isIntervalCountInvalid =
    interval === "DAY" && Number(intervalCount) < 7;
  const minDate = getMinOrderDate(interval, intervalCount);
  const maxDate = getMaxOrderDate(interval, intervalCount);
  ...
  const handleSubmit = async () => {
    if (interval === "DAY" && Number(intervalCount) < 7) {
      setSaveError("Minimum 7 days required for daily delivery.");
      return;
    }
    ...
    const contractDetails = {
      nextOrderDate,
      nextOrderTime,
      nextBillingDateISO: new Date(
        `${nextOrderDate}T${nextOrderTime}:00`,
      ).toISOString(),
      currencyCode,
      billingType,
      intervalCount,
  ...
  useEffect(() => {
    const newMin = getMinOrderDate(interval, intervalCount);
    if (nextOrderDate < newMin) {
      setNextOrderDate(newMin);
    }
  }, [interval, intervalCount]);
    ...
    >
      ...
      <div style={{ marginTop: "16px" }}>
        <Button
          ...
          loading={isSaving}
          disabled={isIntervalCountInvalid}
        >
          Save
        </Button>
      </div>
