...
function CreateSubscription({ currencyCode, shop }) {
  ...
  // ---- contract create fetcher (isi route ke action ko hit karega) ----
  ...
  const handleSubmit = () => {
    ...
    createFetcher.submit(payload, {
      method: "post",
      encType: "application/json",
    });
  };

  // ---- Handle contract-create response ----
  useEffect(() => {
    if (createFetcher.state === 'idle' && createFetcher.data) {
      if (createFetcher.data.success) {
        ...
        setSaveError(createFetcher.data.message || "Something went wrong while saving.");
      }
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [createFetcher.state, createFetcher.data]);

  const isSaving = createFetcher.state === 'submitting' || createFetcher.state === 'loading';
