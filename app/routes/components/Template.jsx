...
useEffect(() => {
  ...
  const validValues = widgetOptions.map((o) => o.value);
  const currentIsValid = widget && validValues.includes(String(widget));

  if (!currentIsValid) {
    setWidget(widgetOptions[0].value);
  }
}, [widgetsLoading, widgetOptions, widget]);
...
{widgetOptions.length === 0 ? (
  ...
  >
    <p style={{ color: "#6d7175", margin: 0 }}>No widgets found</p>
  </div>
) : cardData ? (
  <div
    style={{
