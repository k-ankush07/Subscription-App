...
function Template({ shop, editPlandData, dublicateData }) {
  ...
  //  Yeh track karta hai ki plan already apne DB me save ho chuka hai ya nahi.
  //  editPlandData hone par true (edit mode), warna false (naya create/draft).
  //  Draft ek baar save hone ke baad isse true kar dete hain taaki dubara
  //  "Save as draft" click karne par duplicate entry na bane (create ki jagah update ho).
  const [isPersisted, setIsPersisted] = useState(!!editPlandData);

  //  Current status track karna (UI me dikhane ke liye - optional)
  const [planStatus, setPlanStatus] = useState(editPlandData?.status || "draft");
