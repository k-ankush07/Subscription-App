...
function Template({ shop, editPlandData, dublicateData }) {
  ...
  const [initialSnapshot, setInitialSnapshot] = useState(null);
  // "draft" | "publish" | null - tracks which action is currently saving
  ...
  const handleSubmit = async (e, mode = "publish") => {
    ...
    setSavingMode(mode === "draft" ? "draft" : "publish");
  ...
  useEffect(() => {
    if (fetcher.state === "idle" && fetcher.data) {
      ...
      saveToNodeAPI(payload, oldPlanId).finally(() => setSavingMode(null));
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [fetcher.state, fetcher.data]);
  ...
  return (
    <Frame>
      ...
      <Page
        ...
        backAction={{ content: "Plans", onAction: handleBack, disabled: isSaving }}
        primaryAction={{
          content: editPlandData ? "Update" : "Publish",
          onAction: (e) => handleSubmit(e, "publish"),
          ...
          <form onSubmit={(e) => handleSubmit(e, "publish")}>
            ...
            <Card>
              <Product
                ...
                productError={productError}
              />
              <Button onClick={handleSelectProduct} disabled={isSaving}>
                {selectedProducts.length > 0
                  ? "Add more products"
                  : "Select products"}
              </Button>
