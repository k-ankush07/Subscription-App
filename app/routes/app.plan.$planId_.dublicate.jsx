...
export const loader = async ({ request, params }) => {
  ...
  return json({
    ...
    id: group.id,
    // Duplicate me "Copy of" prefix lagao
    title: `Copy of ${groupMeta.displayTitle || group.merchantCode || ""}`,
    ...
    // Duplicate me sellingPlanId nahi dete — sab naye create honge
    options: group.sellingPlans.edges.map((e) => {
      const plan = e.node;
      const billing = plan.billingPolicy;
      ...
      return {
        // sellingPlanId intentionally nahi — naya create hoga
...
export const action = async ({ request, params }) => {
  ...
  // Duplicate ka DELETE — original ko delete karo
  if (type === "delete") {
    const res = await admin.graphql(
      `mutation sellingPlanGroupDelete($id: ID!) {
        sellingPlanGroupDelete(id: $id) {
  ...
  // CREATE (duplicate = naya plan banana)
  try {
    ...
    const newGroupId = createData.data.sellingPlanGroupCreate.sellingPlanGroup.id;
    const numericId = newGroupId.split("/").pop();
    ...
    return json({ success: true, shopifyGroupId: newGroupId, planId: numericId });
  } catch (error) {
    console.error("Duplicate action error:", error.message);
    return json({ success: false, error: error.message });
  }
};

function PlanDuplicate() {
  const duplicatePlan = useLoaderData();
  return (
    <div>
      <Templates
        shop={duplicatePlan.shop}
        dublicateplanPlanId={duplicatePlan.planId}
        dublicateplanPlanData={duplicatePlan}
        isDuplicate={true}
        singlePlanId={undefined}
      />
    </div>
