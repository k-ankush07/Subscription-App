...
// ---- ACTION: form submit hone par yahan asli subscription contract create hoga (status PAUSED) ----
export async function action({ request }) {
  const { admin } = await authenticate.admin(request);
  ...
  const {
    ...
    nextOrderTime,
    currencyCode,
    intervalCount,   // delivery frequency (number)
  ...
  try {
    ...
    const policyErrors = await setDraftPolicies(admin, draftId, { interval, intervalCount: intervalCountNum });
    ...
    // 5) Draft ko commit karke actual contract bana do (final status PAUSED)
