...
export const loader = async ({ request }) => {
  ...
  const checkoutProfileId =
    data.checkoutProfiles.nodes[0]?.id.split("/").pop();
...
export default function CustomerPortal() {
  ...
  // URL ke according page title
  const pathname = location.pathname;

  let pageTitle = "Customer portal";

  if (pathname === "/app/customer-portal/quick-actions") {
    pageTitle = "Quick actions";
  } else if (
    pathname === "/app/customer-portal/cancellation-reasons"
  ) {
    pageTitle = "Cancellation reason";
  }

  const isRootPage =
    pathname === "/app/customer-portal";
