...
async function loadPlanGroupsAndSettings(admin) {
  const res = await admin.graphql(`
    query LoadPlanGroupsAndSettings {
      shop { id }
      sellingPlanGroups(first: 50) {
        edges {
          node {
...
async function appendAuditLog(admin, shopId, entry) {
  ...
  await admin.graphql(
    `
    mutation MetafieldsSetAuditLog($metafields: [MetafieldsSetInput!]!) {
      metafieldsSet(metafields: $metafields) {
        userErrors { field message }
      }
    }
...
async function findEarliestDueCycle(admin, contractId, now, contractCreatedAt) {
  ...
  try {
    const res = await admin.graphql(
      `
      query FindDueCycles($contractId: ID!, $startDate: DateTime!, $endDate: DateTime!) {
        subscriptionBillingCycles(
          first: 50
          contractId: $contractId
          billingCyclesDateRangeSelector: { startDate: $startDate, endDate: $endDate }
  ...
  try {
    const forwardRes = await admin.graphql(
      `
      query FindUpcomingCycles($contractId: ID!, $startDate: DateTime!, $endDate: DateTime!) {
        subscriptionBillingCycles(
          first: 5
          contractId: $contractId
          billingCyclesDateRangeSelector: { startDate: $startDate, endDate: $endDate }
...
async function processShop(admin) {
  ...
  const contractsRes = await admin.graphql(`
    query ProcessContracts {
      subscriptionContracts(first: 50, query: "status:active OR status:paused") {
        edges {
          node {
            id
            status
  ...
  for (const contract of contracts) {
    ...
    // 🔹 NEW: If contract is PAUSED → auto-skip due cycles, no charging
    if (contract.status === "PAUSED") {
      try {
        const sevenDaysAgo = new Date(
          now.getTime() - 7 * 24 * 60 * 60 * 1000,
        ).toISOString();
        const pausedRes = await admin.graphql(
          `
          query PausedContractDueCycles($contractId: ID!, $startDate: DateTime!, $endDate: DateTime!) {
            subscriptionBillingCycles(
              first: 50
              contractId: $contractId
              billingCyclesDateRangeSelector: { startDate: $startDate, endDate: $endDate }
        ...
        const nodes =
          pausedData.data?.subscriptionBillingCycles?.edges?.map(
            (e) => e.node,
          ) || [];

        const dueCycles = nodes.filter((c) => {
          if (c.skipped) return false;
          if (c.status === "BILLED") return false;
          if (!c.billingAttemptExpectedDate) return false;
          return (
            new Date(c.billingAttemptExpectedDate).getTime() <=
            now.getTime()
          );
        });
        ...
        for (const c of dueCycles) {
          try {
            const skipRes = await admin.graphql(
              `
              mutation SkipSubscriptionBillingCycleForPaused(
            ...
            const payload =
              skipData.data?.subscriptionBillingCycleSkip || null;
            if (payload?.userErrors?.length) {
              console.error(
                `[process-billing-cycles] auto-skip failed for paused contract ${contract.id} cycle ${c.cycleIndex}:`,
                payload.userErrors,
              );
            } else {
              skipped.push({
                ...
                reason:
                  "auto-skipped due cycle while contract is paused",
              });
            }
          } catch (err) {
            console.error(
              `[process-billing-cycles] error auto-skipping paused contract ${contract.id} cycle ${c.cycleIndex}:`,
              err,
            );
          }
        }
        ...
        console.error(
          `[process-billing-cycles] failed to handle paused contract ${contract.id}:`,
          err,
        );
      }

      // Paused contracts ke liye charge/auto-cancel wagaira kuch nahi
      continue;
    }

    // ACTIVE contracts ke liye existing logic
    const { cycle, nextUpcoming } = await findEarliestDueCycle(
      admin,
      contract.id,
      now,
    ...
    const basePriceAmount =
      ...
      firstLineNode?.pricingPolicy?.basePrice?.amount ??
      null;
    const pricingPolicy = contract.lines.edges[0]?.node?.pricingPolicy ?? null;
    const deliveryPriceAmount = contract.deliveryPrice?.amount ?? null;
    const planInfo = sellingPlanId
      ? sellingPlanIdToInfo.get(sellingPlanId)
      : null;
    const groupId = planInfo?.groupId ?? null;
    ...
    try {
      ...
      if (cancelReason) {
        const CANCEL_MUTATION = `
          mutation CancelSubscriptionContractForCron($contractId: ID!) {
            subscriptionContractCancel(subscriptionContractId: $contractId) {
              contract { id status }
              userErrors { field message code }
            }
    ...
    try {
      const chargeRes = await admin.graphql(
        `
        mutation ChargeSubscriptionCycleForCron($contractId: ID!, $index: Int!) {
          subscriptionBillingCycleCharge(
            subscriptionContractId: $contractId
            billingCycleSelector: { index: $index }
          ) {
