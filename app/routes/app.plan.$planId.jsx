...
export const action = async ({ request }) => {
  ...
  //  Draft plans ke paas shopifyGroupId nahi hota (kabhi Shopify pe create hi nahi hua).
  //  Isliye ab error dene ki jagah, agar ID missing hai to niche naya group CREATE karenge.
  ...
  if (!shopifyGroupId) {
    // =====================================================================
    //  DRAFT -> PUBLISH (pehli baar Shopify pe jaa raha hai): naya group CREATE karo
    //  isi tarah jaise createplan.jsx wale action mein hota hai.
    // =====================================================================
    ...
    // =====================================================================
    //  Already published plan ka UPDATE — pehle jaisa hi flow
    // =====================================================================

    //  Plans ko categories mein baanto:
    const plansToUpdate = sellingPlans.filter((sp) => sp.shopifySellingPlanId);
    const plansToCreate = sellingPlans.filter((sp) => !sp.shopifySellingPlanId);
    ...
    //  DB mein jo IDs pehle se saved thi
    const existingDbIds = payload.existingSellingPlanIds || [];
  ...
  // =====================================================================
  //  Common steps — chahe naya group create hua ho ya purana update hua ho:
  //  products/variants ko dobara se attach karna. Naye group ke case me
  //  existing product/variant lists khaali aayengi, isliye remove step
  //  automatically skip ho jaayega.
  // =====================================================================

  //  Existing variants fetch karo
  const existingVariantsRes = await admin.graphql(
    `
    query getExistingVariants($id: ID!) {
      sellingPlanGroup(id: $id) {
  ...
  //  IMPORTANT: ...payload pehle spread karo, shopifyGroupId/shopifySellingPlanIds
  //  ke baad nahi — warna client se aayi purani/khaali shopifyGroupId (draft case
  //  me null) is naye real ID ko overwrite kar degi.
  return Response.json({
    success: true,
    ...payload,
    shopifyGroupId,
