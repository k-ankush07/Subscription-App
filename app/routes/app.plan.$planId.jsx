...
export const loader = async ({ request, params }) => {
  ...
  return json({
    ...
    options: group.sellingPlans.edges.map((e) => {
      ...
      //  Sirf description se data lo
      const d = (() => {
        try { return JSON.parse(plan.description || "{}"); }
        catch { return {}; }
      })();
      ...
      return {
        ...
        deliveryInterval: billing?.interval?.toLowerCase() || "month",
        deliveryFrequency: billing?.intervalCount || 1,
        billingType: d.billingType || "pay_as_you_go",
        billingFrequency: d.billingFrequency || "",
        minOrders: d.minOrders || "disabled",
        maxOrders: d.maxOrders || "unlimited",
        giveDiscount: d.giveDiscount ?? !!pricing,
        discountType:
          d.discountType ??
          (adjustmentValue?.percentage != null ? "percentage" : "fixed"),
        discountAmount:
          ...
          adjustmentValue?.amount ??
          0,
        giveShippingDiscount: d.giveShippingDiscount ?? false,
        changeDiscountAfter: d.changeDiscountAfter ?? false,
        discountAmount2: d.discountAmount2 ?? "",
        afterOrders: d.afterOrders ?? "",
        discountType2: d.discountType2 ?? "amount",
        shippingDiscount: d.shippingDiscount ?? "",
        shippingAfterOrders: d.shippingAfterOrders ?? "",
        shippingDiscountType: d.shippingDiscountType ?? "fixed",
        changeQtyAfterOrders: d.changeQtyAfterOrders ?? false,
        changeQtyAfterOrdersNum: d.changeQtyAfterOrdersNum ?? "",
        changeQtyQuantity: d.changeQtyQuantity ?? "",
        changeQtyProducts: d.changeQtyProducts ?? [],
        removeFreeProducts: d.removeFreeProducts ?? false,
        removeFreeAfterOrders: d.removeFreeAfterOrders ?? "",
        removeFreeProductsList: d.removeFreeProductsList ?? [],
        setMinQty: d.setMinQty ?? false,
        minQuantity: d.minQuantity ?? "",
        allowAutoActions: d.allowAutoActions ?? false,
        automationCycles: d.automationCycles ?? [],
        selectedProducts: d.selectedProducts ?? [],
      };
    }),
    productChanges: (() => {
      try {
        //  Group level description se productChanges lo
        const groupDesc = JSON.parse(group.description || "{}");
        return groupDesc.productChanges ?? { swap: true, variant: true, quantity: true, keepDiscount: true };
      } catch {
        return { swap: true, variant: true, quantity: true, keepDiscount: true };
      }
    })(),
  });
};
  ...
  try {
    ...
    `, {
      variables: {
        ...
        input: {
          name: planPayload.title,
          //  Group level description mein productChanges save karo
          ...
          sellingPlansToUpdate: planPayload.options
            ...
            .map(opt => {
              ...
              return {
                id: opt.sellingPlanId,
                name: opt.name || "Option",
                //  description mein saara custom data
                description: JSON.stringify({
