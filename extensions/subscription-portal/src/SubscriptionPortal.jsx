...
function SubscriptionDetail({
  ...
  const [address, setAddress] = useState(sub.deliveryMethod?.address ?? null);
  const addressModalRef = useRef(null);
  const addressHideBtnRef = useRef(null); // NEW — hidden command="--hide" button
  ...
  const [cancelLoading, setCancelLoading] = useState(false);
  const cancelModalRef = useRef(null);
  const cancelHideBtnRef = useRef(null); // NEW — hidden command="--hide" button
  ...
  const [rescheduleError, setRescheduleError] = useState(null);
  const rescheduleModalRef = useRef(null); // outer top-level "Reschedule" button ke liye
  const rescheduleHideBtnRef = useRef(null); // NEW — hidden command="--hide" button
  const upcomingModalRef = useRef(null);
  ...
  async function handleCancel() {
    ...
    try {
      ...
      // Modal ko reliably band karne ke liye — ref.hide() ke bajaye
      // hidden command="--hide" button click karo
  ...
  const addressModalId = `edit-address-modal-${numericId}`;
  const rescheduleModalId = `reschedule-modal-${numericId}`;
  const cancelModalId = `cancel-modal-${numericId}`;
  const cycles = upcomingCycles;
  const visibleCycles = cycles.slice(0, VISIBLE_CYCLES_LIMIT);
      ...
      <s-section>
        <s-stack direction="block" gap="base">
          ...
          >
            ...
            {/* Hidden helper button — clicked programmatically on cancel success
                to reliably close the modal via the native command API */}
            <button
          ...
          {status !== "CANCELLED" && status !== "EXPIRED" && (
            <s-box border="base" borderRadius="base" padding="base">
              ...
              >
                ...
                {/* Hidden helper button — clicked programmatically on save success
                    to reliably close the modal via the native command API */}
                <button
