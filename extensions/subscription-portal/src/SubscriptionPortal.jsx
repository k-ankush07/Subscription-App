...
function Extension() {
  ...
  // Skip/unskip backend ko hit karta hai. contractId (gid ya numeric dono chalega,
  // backend khud numeric me convert kar leta hai) aur billing cycle index leta hai.
  async function runCycleAction(action, contractId, cycleIndex) {
    const token = await shopify.sessionToken.get();
    const res = await fetch(`${API_BASE}/api/subscriptions/${action}`, {
      method: "POST",
      headers: {
      ...
      body: JSON.stringify({
        contractId: getNumericId(contractId) || contractId,
        cycleIndex,
      }),
    });
    if (!res.ok) {
  ...
  // Skip/unskip ke baad list ko re-fetch karke usi contract ka fresh data
  // selectedSub me daal deta hai. Isse UI turant update hoti hai AUR refresh
  // pe bhi persist rehta hai, kyunki data ab actually Shopify pe saved hai —
  // pehle yahi refetch missing tha isliye skip "wapas aa jaata tha".
  const refreshSubscription = useCallback(
    async (contractId) => {
      const fresh = await fetchPage({ afterCursor: null, reset: true });
      const updated = fresh?.find((s) => s.id === contractId) || null;
  ...
  async function handleSkip(contractId, cycleIndex) {
    try {
      await runCycleAction("skip", contractId, cycleIndex);
      await refreshSubscription(contractId);
    } catch (err) {
      console.error("Skip failed:", err);
      shopify.toast?.show?.("Could not skip this order", { isError: true });
    }
  }

  async function handleUnskip(contractId, cycleIndex) {
    try {
      await runCycleAction("unskip", contractId, cycleIndex);
      await refreshSubscription(contractId);
    } catch (err) {
      console.error("Unskip failed:", err);
      shopify.toast?.show?.("Could not unskip this order", { isError: true });
    }
  }
...
function SubscriptionDetail({ sub, onBack, onSkip, onUnskip }) {
  const items = sub.nextOrderLineItems?.length
    ? sub.nextOrderLineItems
    : (sub.lines?.edges?.map((e) => e.node) ?? []);
      ...
      <s-section>
        <s-stack direction="block" gap="base">
          ...
          {(() => {
            ...
            const modalId = `upcoming-orders-modal-${numericId}`;
            const cycles = sub.upcomingCycles ?? [];
            // "Upcoming order" card ke Skip button ke liye — sabse pehla
            // aisa cycle jo abhi skip nahi hua, wahi asli "next" order hai.
            const currentCycle = cycles.find((c) => !c.skipped) || null;
                ...
                <s-stack direction="block" gap="base">
                  ...
                  <s-stack direction="inline" gap="tight">
                    <s-button
                      ...
                      onClick={() =>
                        currentCycle &&
                        onSkip(sub.id, currentCycle.cycleIndex)
                      }
                    >
                      Skip
                    </s-button>
                  ...
                  <s-stack direction="block" gap="base">
                    {cycles.map((cycle) => (
                      <s-stack
                        ...
                        justifyContent="space-between"
                      >
                        <s-text tone={cycle.skipped ? "subdued" : undefined}>
                          {formatShortWithYear(
                            toDateOnlyString(cycle.billingAttemptExpectedDate),
                          )}
                        ...
                        {cycle.skipped ? (
                          <s-link
                            onClick={() =>
                              onUnskip(sub.id, cycle.cycleIndex)
                            }
                          >
                            Unskip
                          </s-link>
                          ...
                          <s-stack direction="inline" gap="tight">
                            <s-link
                              onClick={() =>
                                onSkip(sub.id, cycle.cycleIndex)
                              }
                            >
                              Skip
                            </s-link>
