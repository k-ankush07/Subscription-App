...
async function processShopBillingCycles(admin, shop) {
  ...
  for (const contract of contracts) {
    try {
      ...
      // Look ahead at the next 3-4 billing cycles for this contract instead
      // of only whatever falls inside a tight date window.
      const upcomingCycles = await getUpcomingBillingCycles(admin, contract.id, 4);

      console.log(
        `Contract ${contract.id} upcoming cycles:`,
        upcomingCycles.map(
          (c) => `#${c.cycleIndex} status=${c.status} skipped=${c.skipped} date=${c.billingAttemptExpectedDate}`,
        ),
      );

      // Out of those 3-4 fetched cycles, only the ones actually due right
      // now (unbilled, not skipped, inside the -5min/+24hr window) get charged.
      const dueCycles = filterDueCycles(upcomingCycles, startDate, endDate);
...
// Fetch the next N billing cycles for a contract (no date-range restriction),
// so we can look ahead 3-4 cycles instead of just whatever falls inside a
// narrow date window. Shopify returns cycles starting from the current/next
// pending one onward when no billingCyclesDateRangeSelector is passed.
async function getUpcomingBillingCycles(admin, contractId, count = 4) {
  const response = await admin.graphql(
    `
    query UpcomingBillingCycles($contractId: ID!, $first: Int!) {
      subscriptionBillingCycles(first: $first, contractId: $contractId) {
        edges {
          node {
            cycleIndex
    ...
    `,
    { variables: { contractId, first: count } },
  );
...
// In-memory filter: out of a list of fetched cycles, keep only the ones
// that are actually due to be charged right now (same window logic that
// was previously baked into the GraphQL date-range selector).
function filterDueCycles(cycles, startDate, endDate) {
  const start = new Date(startDate).getTime();
  const end = new Date(endDate).getTime();
  return cycles.filter((cycle) => {
    if (cycle.skipped || cycle.status !== "UNBILLED") return false;
    const due = new Date(cycle.billingAttemptExpectedDate).getTime();
