...
async function processShopBillingCycles(admin, shop) {
  ...
  for (const contract of contracts) {
    try {
      ...
      // Ab yaha 4 upcoming cycles fetch honge (preview/lookahead ke liye)
      const upcomingCycles = await getUpcomingBillingCycles(admin, contract.id, 4);

      // In-memory filter se sirf wahi cycles jo abhi due hain, charge honge
      const dueCycles = filterDueCycles(upcomingCycles, startDate, endDate);

      console.log(
        `Contract ${contract.id}: upcoming=`,
        upcomingCycles.map((c) => `${c.cycleIndex}:${c.status}:${c.billingAttemptExpectedDate}`),
      );
