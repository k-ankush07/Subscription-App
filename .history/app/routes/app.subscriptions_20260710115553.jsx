...
// Type-specific renderer — QUANTITY_CHANGE aur swap ke fields alag hain,
// isliye har type ko uske apne fields se render karna zaroori hai.
function renderActionDetails(a) {
  switch (a.type) {
    case "QUANTITY_CHANGE": {
      const productTitles =
        Array.isArray(a.products) && a.products.length > 0
          ? a.products.map((p) => p.title).join(", ")
          : null;
      return (
        <>
          {productTitles && <>{productTitles} — </>}
          {a.value !== undefined && a.value !== "" ? (
            `new quantity: ${a.value}`
          ) : (
            <span style={{ color: "#991b1b" }}>⚠️ no value set</span>
          )}
        </>
      );
    }
    ...
    case "swap": {
      const sourceName = a.sourceProductName;
      const destNames =
        Array.isArray(a.dests) && a.dests.length > 0
          ? a.dests.map((d) => d.name).join(", ")
          : null;

      if (!sourceName && !destNames) {
        return <span style={{ color: "#991b1b" }}>⚠️ no product set</span>;
      }

      return (
        <>
          {sourceName || "Unknown product"}
          {" → "}
          {destNames || (
    ...
    default: {
      if (a.productTitle || a.variantTitle) {
        return (
          <>
      ...
      return <span style={{ color: "#991b1b" }}>⚠️ no product/value set</span>;
    }
  }
}
...
export default function SubscriptionDetailPage() {
  ...
  const willApply = Array.isArray(preview.nextOrder.willApply)
    ...
    : [];
    console.log("njksbfjsdbfksdbfskf",willApply)
  return (
