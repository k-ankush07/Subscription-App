...
export async function action({ request, params }) {
  ...
  if (
    ...
    if (type === "unskip") {
      ...
      const res = await admin.graphql(
        `
        mutation ChargeSubscriptionBillingCycleByIndex(
          $contractId: ID!
          $cycleIndex: Int!
        ) {
      ...
      if (!payload || payload.userErrors?.length) {
        console.error("Charge failed", payload?.userErrors);
        return {
          success: false,
          error:
            payload?.userErrors?.map((e) => e.message).join(", ") ||
      ...
      const stateType = payload.subscriptionBillingAttempt?.state?.__typename;
