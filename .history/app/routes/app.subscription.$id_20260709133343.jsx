  ...
  if (
    ...
    if (type === "unskip") {
      ...
      return {
        ...
        if (type === "chargeCycle") {
      const cycleIndexStr = formData.get("cycleIndex");
      const cycleDate = formData.get("cycleDate");

      if (!cycleDate) {
        return {
          success: false,
          action: "chargeCycle",
          error: "Missing billing cycle date",
        };
      }

      const idempotencyKey = crypto.randomUUID
        ? crypto.randomUUID()
        : `${contractId}-${cycleIndexStr}-${Date.now()}`;

      const res = await admin.graphql(
        `
        mutation ChargeSpecificBillingCycle(
          $contractId: ID!
          ...
          subscriptionBillingAttemptCreate(
            ...
            subscriptionBillingAttemptInput: {
              ...
              billingCycleSelector: { date: $cycleDate }
            }
          ) {
      ...
      if (!payload || payload.userErrors?.length) {
        console.error("Charge cycle failed", payload?.userErrors);
        return {
          success: false,
          action: "chargeCycle",
          error:
      ...
      const orderId =
        stateType === "SubscriptionBillingAttemptSuccessState"
          ? attempt.state.order?.id
          : null;
      ...
      return {
        ...
        stateType,
        orderId,
        cycleIndex: Number(cycleIndexStr),
      };
    }
  }
