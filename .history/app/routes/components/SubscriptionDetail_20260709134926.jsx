...
export default function SubscriptionDetail() {
  const { contract, upcomingCycles, internalNotes, customerNotes } =
    useLoaderData();
  console.log("upcoming", contract,contract);
        ...
        {contract?.status !== "CANCELLED" && (
          <Card>
            ...
            {upcomingCycles?.map((cycle, index) => {
              ...
              let showChargeButton = false;
              if (cycleDate) {
                const diffMs = cycleDate.getTime() - now.getTime();
                const diffHours = diffMs / (1000 * 60 * 60); // ms -> hours

                // condition: within next 24 hours (and not in the past)
                showChargeButton = diffHours <= 24 && diffHours >= 0;
              }
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    {contract?.status === "ACTIVE" && !cycle.skipped && (
                      <>
                        {/* Skip / Resume buttons same as before */}
                        <Button
                          plain
                          onClick={() => {
                            fetcher.submit(
                    ...
                    {/* NEW: charge button visible only if within 24h */}
                    {contract?.status === "ACTIVE" &&
                      !cycle.skipped &&
                      showChargeButton && (
                        <Button
